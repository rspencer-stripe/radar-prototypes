...
async function getSettings() {
  ...
  const { rows } = await sql`SELECT authors, tags FROM app_settings WHERE id = 1`;
  const row = rows[0] || {};
  return { authors: row.authors || DEFAULT_AUTHORS, tags: row.tags || DEFAULT_TAGS };
}

async function updateSettings({ authors, tags }) {
  await ensureSettings();
